...
exports.handler = async (event) => {
  ...
  try {
    ...
    const rows = data.slice(1).map(r => r.map(v => v === null || v === undefined ? '' : String(v).trim()));
    ...
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      const folio = i + 1;
      const nombreRaw = val(row[1]);
      const apellido1Raw = val(row[2]);
      const rutRaw = val(row[4]) || `sin_rut_${i}`;
      ...
      const nombreSafe = normStr(nombreRaw);
      const apellidoSafe = normStr(apellido1Raw);
      const rutSafe = rutRaw.replace(/[^a-zA-Z0-9]/g, '');
    ...
    return {
      ...
      body: JSON.stringify({ zipBase64, total: rows.length })
    };
